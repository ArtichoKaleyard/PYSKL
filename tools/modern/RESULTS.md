...
10-clip offset 贡献、greedy 子集、以及正收益 offset 与 KNS-v1 峰值/补点采样的
关系已集中整理到 `tools/modern/ANALYSIS.md`。简要结论是：两个数据集都存在
小 offset 子集优于完整 10-clip 的现象；进一步区分分类正确性和 KNS-v1
峰值/补点后，10-clip 的收益更像来自 uniform 相位互补，而不是速度/加速度峰值本身。
...
- partial score 文件只用于长任务进度检查，最终指标以非 partial 的 score 和
  metrics 文件为准。
- 本文档目前只覆盖官方权重测试、PoseC3D 分数融合和 E/F 组 test-time sampling；
  额外分析实验集中记录在 `tools/modern/ANALYSIS.md`，训练阶段采样实验尚未开始。
